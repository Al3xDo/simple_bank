...
func Prepare(ctx context.Context, db DBTX) (*Queries, error) {
	...
	if q.listEntriessStmt, err = db.PrepareContext(ctx, listEntriess); err != nil {
		return nil, fmt.Errorf("error preparing query ListEntriess: %w", err)
	}
...
func (q *Queries) Close() error {
	...
	if q.listEntriessStmt != nil {
		if cerr := q.listEntriessStmt.Close(); cerr != nil {
			err = fmt.Errorf("error closing listEntriessStmt: %w", cerr)
		}
	}
...
func (q *Queries) WithTx(tx *sql.Tx) *Queries {
	return &Queries{
		...
		listEntriessStmt:        q.listEntriessStmt,
